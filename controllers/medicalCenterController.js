...
const registerMedicalCenter = async (req, res) => {
  try {
    ...
    // -----------------------------
    // Basic validation (updated for new address schema)
    // -----------------------------
   if (
  !facility_name ||
  !official_domain_email ||
...
const updateProfile = async (req, res) => {
 ...
 if (updates.address) {
  const hasLatLng =
    typeof updates.address.lat === 'number' &&
    Number.isFinite(updates.address.lat) &&
    typeof updates.address.lng === 'number' &&
    Number.isFinite(updates.address.lng);

  const hasGooglePlaceData =
    typeof updates.address.place_id === 'string' &&
    updates.address.place_id.trim().length > 0 &&
    typeof updates.address.formatted_address === 'string' &&
    updates.address.formatted_address.trim().length > 0 &&
    hasLatLng;

  let finalLocationSource = 'address';

  if (updates.address.location_source === 'address_and_geolocation') {
    finalLocationSource = 'address_and_geolocation';
  } else if (updates.address.location_source === 'geolocation') {
    finalLocationSource = 'geolocation';
  }

  updates.address = {
    full_address: updates.address.full_address?.trim() || '',
    formatted_address: updates.address.formatted_address?.trim() || '',
    place_id: updates.address.place_id?.trim() || '',
    lat: hasLatLng ? updates.address.lat : null,
    lng: hasLatLng ? updates.address.lng : null,
    location_source: finalLocationSource,
    is_location_verified: hasGooglePlaceData || finalLocationSource === 'geolocation'
  };
}
